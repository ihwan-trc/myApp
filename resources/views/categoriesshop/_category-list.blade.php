@foreach ($categories as $category)
    ...
    <li class="list-group-item list-group-item-action d-flex justify-content-between align-items-center pr-0">
        ...
        <!-- detail -->
        @can('category_detail')
            <a href="{{ route('categoriesshop.show',['category' => $category]) }}" class="btn btn-sm btn-primary" role="button">
                <i class="fas fa-eye"></i>
            </a>
        @endcan
        <!-- edit -->
        @can('category_update')
            <a href="{{ route('categoriesshop.edit', ['category' => $category]) }}" class="btn btn-sm btn-info" role="button">
                <i class="fas fa-edit"></i>
            </a>
        @endcan
        <!-- delete -->
        @can('category_delete')
            <form class="d-inline" action="{{ route('categoriesshop.destroy', ['category' => $category]) }}" role="alert" method="POST" 
